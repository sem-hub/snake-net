...
func decodeData(data []byte) (uint16, string) {
	logger := configs.GetLogger("icmp")
	...
	var port uint16
	_, err = binary.Decode(decBuf, binary.BigEndian, &port)
	if err != nil {
		logger.Error("Error decoding port from data", "error", err)
		return 0, ""
	}
...
func StartICMPCommonListen(isIPv4 bool) {
	...
	if isIPv4 {
		...
		if err := ipv4Conn.SetControlMessage(ipv4.FlagDst|ipv4.FlagInterface, true); err != nil {
			logger.Warn("Failed to enable IPv4 control messages", "error", err)
		}
	} else {
		ipv6Conn = icmpConn.IPv6PacketConn()
		if err := ipv6Conn.SetControlMessage(ipv6.FlagDst|ipv6.FlagInterface, true); err != nil {
			logger.Warn("Failed to enable IPv6 control messages", "error", err)
		}
	}
	...
	for {
		...
		if isIPv4 {
			...
			n, cm, peer, err = ipv4Conn.ReadFrom(buf)
			if cm != nil {
				replyIP = cm.Dst
			}
		} else {
			var cm *ipv6.ControlMessage
			n, cm, peer, err = ipv6Conn.ReadFrom(buf)
			if cm != nil {
				replyIP = cm.Dst
			}
		}
		...
		case ipv4.ICMPTypeEcho, ipv6.ICMPTypeEchoRequest:
			...
			if isIPv4 {
				if replyIP != nil {
					cm := &ipv4.ControlMessage{Src: replyIP}
					if _, err := ipv4Conn.WriteTo(replyBytes, cm, peer); err != nil {
				...
				if replyIP != nil {
					cm := &ipv6.ControlMessage{Src: replyIP}
					if _, err := ipv6Conn.WriteTo(replyBytes, cm, peer); err != nil {
						logger.Error("Error sending ICMP Echo Reply", "error", err)
						continue
					}
				} else {
		...
		case ipv4.ICMPTypeEchoReply, ipv6.ICMPTypeEchoReply:
			...
			if echo, ok := rm.Body.(*icmp.Echo); ok {
				if echo.ID == id && echo.Seq == 1 {
					...
					port, protocol := decodeData(echo.Data)
					if port != 0 {
						logger.Debug("Received server info", "peer", peer, "port", port, "protocol", protocol)
						return port, protocol
					}
					// Ignore the packet
					continue
